...
module qsys_top_r1_altera_merlin_axi_slave_ni_1941_pedgx2q
#(
    // -----------------------------------------------
...
(
    ...
    reg [INTERNAL_ID_W -1 :0 ] awid_internal; //to solve qis warnings
    reg [INTERNAL_ID_W -1 :0 ] wid_internal ; //to solve qis warnings
    reg [INTERNAL_ID_W -1 :0 ] arid_internal; //to slove qis warnings
   ...
   endgenerate
	...
	generate
		if (( SLAVE_ID_W + THREAD_ID_W ) > AXI_SLAVE_ID_W ) begin    
			assign write_rp_data_bid_internal  = {{MATCH_ID_W{1'b0}}, bid};
		end else begin	  
			assign write_rp_data_bid_internal  = bid;
		end
	endgenerate 
	...
	write_rp_data[PKT_ORI_BURST_SIZE_H:PKT_ORI_BURST_SIZE_L]	= write_rsp_fifo_outdata[PKT_ORI_BURST_SIZE_H:PKT_ORI_BURST_SIZE_L];	
       if (PASS_ID_TO_SLAVE) begin
            { write_rp_data[PKT_DEST_ID_H:PKT_DEST_ID_L], write_rp_data[PKT_THREAD_ID_H:PKT_THREAD_ID_L] }  = write_rp_data_bid_internal;
        end
   ...
   generate
      if (USE_MEMORY_BLOCKS) begin : Using_M20k_with_2_clk_latency
          assign bready                 = (write_rp_ready && write_rsp_fifo_outvalid) || (bvalid && write_rsp_fifo_posted && write_rsp_fifo_outvalid) ||  bvalid && write_rsp_fifo_outvalid && !write_rsp_fifo_posted && !write_rsp_fifo_eop;  
...
//to deal with case where MATCH_ID_W padding is required/not required	
    generate
	if (( SLAVE_ID_W + THREAD_ID_W ) > AXI_SLAVE_ID_W ) begin
		assign read_rp_data_rid_internal = {{MATCH_ID_W{1'b0}},rid};
	end else begin
            	assign read_rp_data_rid_internal       = rid;
	end
    endgenerate	
    ...
    always_comb
	    ...
	    read_rp_data[PKT_ORI_BURST_SIZE_H:PKT_ORI_BURST_SIZE_L] = read_rsp_fifo_outdata[PKT_ORI_BURST_SIZE_H:PKT_ORI_BURST_SIZE_L];
            ...
            if (PASS_ID_TO_SLAVE) begin
                { read_rp_data[PKT_DEST_ID_H:PKT_DEST_ID_L], read_rp_data[PKT_THREAD_ID_H:PKT_THREAD_ID_L] } = read_rp_data_rid_internal;
            end
...
// start of packet assertion should only happen at the start of burst and should not be asserted at the start of sub-burst
    ...
    generate
      if (USE_MEMORY_BLOCKS) begin : Using_M20k_change2
          assign rready           = read_rp_ready && read_rsp_fifo_outvalid;
   ...
   generate
      if (USE_MEMORY_BLOCKS) begin : Using_M20k_rvalid
          assign rvalid_uncompressor = (rvalid && read_rsp_fifo_outvalid);
      end else begin : Using_ALM_memory_rvalid
          assign rvalid_uncompressor = rvalid;
      end
   endgenerate
    ...
    qsys_top_r1_altera_merlin_axi_slave_ni_altera_avalon_sc_fifo_1941_ghdm7ly #(
        .SYMBOLS_PER_BEAT    (1),
   ...
   generate
      if (USE_MEMORY_BLOCKS) begin : Using_M20k_change5
           ERROR_readdata_receive_but_read_ID_fifo_is_empty:
               assert property ( @(posedge aclk)
                   disable iff (areset) (!read_rsp_fifo_outvalid |-> !(rvalid && read_rsp_fifo_outvalid))
               );
